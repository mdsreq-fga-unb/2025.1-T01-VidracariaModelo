...
| :- | :- | :- | :- | :- |
|**RF01**|Realizar agendamento|O sistema deve permitir a seleção de data e horário disponíveis para marcar um compromisso|**RN** **02**,**03**,**04**,**06**,**31**,**47**|Objetivo 2: Otimizar gestão de orçamentos e agendamentos.|
|**RF02**|Visualizar serviços agendados|O sistema deve listar os agendamentos realizados para os administradores visualizarem os compromissos do dia.|**RN** **01**, **07**, **08**, **09**, **10**, **11**, **12**, **13**|Objetivo 2: Otimizar gestão de orçamentos e agendamentos.|
|**RF03**|Remarcar agendamento|O sistema deve permitir a remarcação de agendamentos para programar compromissos com clientes.|**RN** **14**, **16**, **19**, **20**, **21**, **22**|Objetivo 2: Otimizar gestão de orçamentos e agendamentos.|
|**RF04**|Cancelar agendamento|O sistema deve permitir o cancelamento dos agendamentos para ajustes na agenda de compromissos.|**RN** **23**, **26**, **27**, **28**, **30**|Objetivo 2: Otimizar gestão de orçamentos e agendamentos.|
|**RF05**|Criar orçamento|O sistema deve permitir a criação de orçamentos para registrar propostas de serviços no site.|**RN** **32**, **34**, **35**, **36**, **73** |Objetivo 2: Otimizar gestão de orçamentos e agendamentos.|
|**RF06**|Listar orçamentos|O sistema deve listar orçamentos solicitados para facilitar o acompanhamento das propostas.|**RN** **37**, **38**, **40**, **41**, **42** |Objetivo 2: Otimizar gestão de orçamentos e agendamentos.|
|**RF07**|Editar orçamentos|O sistema deve permitir edição de valores, prazos e observações para atualizar os dados do orçamento conforme necessário.|**RN** **43**, **44** |Objetivo 2: Otimizar gestão de orçamentos e agendamentos.|
|**RF08**|Criar Venda|O sistema deve permitir o registro de uma venda.|**RN** **56**, **73** |Objetivo 4: Aprimorar gestão financeira e de vendas.|
|**RF09**|Listar vendas|O sistema deve apresentar a lista de vendas feitas pela empresa para controle comercial.|**RN** **49**, **50**, **51**, **52** |Objetivo 4: Aprimorar gestão financeira e de vendas.|
|**RF10**|Visualizar venda|O sistema deve permitir a visualização dos detalhes da venda para consulta de informações.|**RN** **53**, **54**  |Objetivo 4: Aprimorar gestão financeira e de vendas.|
|**RF11**|Editar venda|O sistema deve permitir edição dos detalhes da venda para correção de dados.|**RN** **49**, **60** |Objetivo 4: Aprimorar gestão financeira e de vendas.|
|**RF12**|Registrar recibo|O sistema deve permitir o registro de recibos com dados do serviço prestado para gerar comprovantes.|**RN** **62**, **63**, **64**, **65** |Objetivo 4: Aprimorar gestão financeira e de vendas.|
|**RF13**|Editar recibo|O sistema deve permitir alteração dos dados registrados em um recibo para correções.|**RN** **57**, **65**, **66**, **72**|Objetivo 4: Aprimorar gestão financeira e de vendas.|
|**RF14**|Exportar recibo|O sistema deve gerar arquivo PDF com os dados do recibo preenchido para emissão de comprovantes.|**RN** **58**, **67**, **68**, **69**, **70**, **71**, **72**|Objetivo 4: Aprimorar gestão financeira e de vendas.|
|**RF15**|Registrar despesas|O sistema deve permitir que usuários registrem novas despesas para controle financeiro.|**RN** **57**, **74**, **75**, **76**, **77**, **78**|Objetivo 4: Aprimorar gestão financeira e de vendas.|
|**RF16**|Editar despesas|O sistema deve permitir edição de despesas conforme as permissões para correção ou atualização de registros.|**RN** **79**, **80** |Objetivo 4: Aprimorar gestão financeira e de vendas.|
|**RF17**|Exportar despesas|O sistema deve gerar relatório em PDF e XLS com os dados registrados das despesas para controle e análise financeira.|**RN** **83**, **84**, **85**, **86** |Objetivo 4: Aprimorar gestão financeira e de vendas.|
|**RF18**|Registrar interações com cliente|O sistema deve registrar um log de serviços já requisitados pelo cliente contendo informações sobre pagamento e detalhes do serviço prestado para acompanhamento do histórico de atendimento.|**RN** **87**, **88**, **89** |Objetivo 4: Aprimorar gestão financeira e de vendas.|
|**RF19**|Contatar empresa|O sistema deve permitir comunicação por WhatsApp através de um link direto, formulário do Google e link direto para redes sociais (Instagram) para facilitar o contato com os clientes.|**RN** **90**, **91**, **92** |Objetivo 1: Sofisticar o marketing e proporcionando posicionamento online. |
|**RF20**|Cadastrar perguntas frequentes na FAQ|O sistema deve permitir que os administradores adicionem as perguntas frequentes na área de FAQ para auxiliar o cliente.|**RN** **29**, **39**, **93** |Objetivo 1: Sofisticar o marketing e proporcionando posicionamento online. |
...
| **RN05** | O sistema deve confirmar o agendamento por meio de notificação no Email. |
...
| **RN15** | Apenas o administrador que criou o agendamento pode realizar a remarcação. |
...
| **RN17** | Apenas o perfil do cliente deve ser excluído, preservando o log de interações. |
| **RN18** | Os gráficos gerados pelo sistema devem ser do tipo “barras”. |
...
| **RN24** | A avaliação deve conter obrigatoriamente: nota (ex.: de 1 a 5 estrelas ou pontos) e comentário com no mínimo 20 caracteres. |
| **RN25** | O comentário deve ter um limite máximo de, por exemplo, 500 caracteres para garantir objetividade. |
...
| **RN29** | A pergunta deve ter no mínimo 10 caracteres, e a resposta, no mínimo 20 caracteres, para garantir clareza e completude. |
| **RN30** | Toda exclusão de avaliação deve ser registrada com uma justificativa, seja ela: conteúdo inadequado, comentário ofensivo, informação falsa ou solicitação do próprio cliente. |
| **RN31** | O sistema deve fornecer notificações sempre que um novo agendamento for feito para garantir acompanhamento. |
| **RN32** | Orçamentos devem estar associados a um cliente e podem ser filtrados por cliente, status, etc. |
| **RN33** | Um orçamento só pode ser criado se contiver: CPF, nome completo do cliente, email, endereço e observações gerais (opcional). |
...
| **RN39** | Cada item de FAQ deve conter obrigatoriamente: pergunta (título), resposta (conteúdo), categoria (opcional, para organização por temas). |
...
| **RN45** | Os agendamentos podem ser cancelados por administradores e pelos clientes. |
| **RN46** | Apenas os administradores podem criar, editar ou excluir perfis de cliente. |
| **RN47** | O sistema deve exibir calendário com dias e horários já ocupados e disponíveis para agendamento. |
| **RN48** | Vendas e recibos devem estar relacionados a um cliente. |
...
| **RN55** | Clientes podem avaliar serviços após sua conclusão. |
...
| **RN59** | O cliente pode solicitar a exclusão da própria avaliação, mediante autenticação e confirmação. |
| **RN60** | Sempre que os itens ou valores forem alterados, o sistema deve recalcular automaticamente. |
| **RN61** | Somente administradores podem incluir ou editar anotações internas no perfil do cliente. |
